import datetime
import sqlite3
from matplotlib import pyplot as plt
import calendar


date = str(datetime.datetime.now() + datetime.timedelta(hours=3))[:-16]
print(date)


def get_data(command):
    if command == '/btc1' or command == '/btc5':
        db_name = 'DbOf2HourValues.db'
        if command == '/btc1':
            limit = 12
            width = 15
            height = 6
            fontsizex = 7
            fontsizey = 13
        else:
            limit = 60
            width = 20
            height = 6
            fontsizex = 10
            fontsizey = 13

    if command == '/btcm' or command == '/btcy':
        db_name = 'DbOf24HourValues.db'
        if command == '/btcm':
            limit = 30
            width = 20
            height = 6
            fontsizex = 10
            fontsizey = 13
        else:
            limit = 365
            width = 40
            height = 10
            fontsizex = 20
            fontsizey = 25

    def get_data_from_db():
        conn = sqlite3.connect('{}'.format(db_name))

        c = conn.cursor()

        c.execute("SELECT * FROM BTC ORDER BY {} DESC LIMIT {}".format(date, limit))

        output_data = c.fetchall()
        conn.close()

        x, y = [], []
        for line in open('/home/alaskastorm/vim_projects/parsers/btc_with_graph/dataa.csv', 'r'):
            values = [s for s in line.split('",')]
            x.append(values[0][1:])
            y.append(float(values[1][:-1]))
            if len(x) == 12:
                break
        return x, y

        # x, y = [], []
        #
        # counter = 0
        #
        # for i in output_data:
        #     xvalue = output_data[counter][0]
        #     yvalue = output_data[counter][1]
        #     x.append(xvalue)
        #     y.append(float(yvalue))
        #     counter += 1
        # return x, y

    def draw_graph():
        x = get_data_from_db()[0]
        y = get_data_from_db()[1]

        if limit == 12:
            positions_on_y = [i for i in range(12)]
            positions_names = x[::1]
            rotation = 0
        if limit == 30:
            positions_on_y = [i for i in range(30)]
            positions_names = x[::1]
            rotation = 20
        if limit == 60:
            positions_on_y = [i for i in range(0, 60, 5)]
            positions_names = x[::5]
            rotation = 0
            print(positions_names)
        if limit == 365:
            positions_on_y = [0, 31, 60, 91, 121, 152, 182, 213, 243, 274, 304, 335]
            # positions_names = [x[0], x[31], x[60], x[91], x[121], x[152],
            #                    x[182], x[213], x[243], x[274], x[304], x[335]]
            positions_names = calendar.month_name[1:13]
            rotation = 0

        plt.figure(figsize=(width, height))

        ax = plt.axes()
        ax.set_facecolor('#36393f')

        plt.plot(x, y, linestyle='solid', marker='o', color='#46f148')
        plt.yticks(fontsize=fontsizey, color='white')
        plt.xticks(positions_on_y,
                   positions_names,
                   color='white',
                   fontsize=fontsizex,
                   rotation=rotation)
        plt.savefig('test1.jpeg', facecolor='#2f3136')
        photo = open('test1.jpeg', 'rb')
        return photo
    return draw_graph()


print(get_data(command='/btc1'))




